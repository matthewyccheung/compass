...
config = {
    ...
    'device': 'cuda:5',
    'device2': 'cuda:6',
    'dataset': 'kvasir',  # Options: 'ebhi', 'ham10k', 'kvasir', 'tn3k'
    'arch': 'UNet',     # Options: 'UNet', 'SegResNet'
    'metric': 'mask_sum',    # Scalar target (default): sum of mask pixels
    'generate_vars': True, # Set to False to skip data extraction if already done
    'overwrite': True,
...
forward_x, forward_latent, compute_jacobian_logits = get_fns(arch + 'Logits', post_trans_diff)
...
if config['generate_vars']:
    print("Extracting variables for COMPASS-L (logits-only)...")
    DataExtractionWrapperSave(loaders, data_dir, model, config['channel'],
                              forward_x, forward_latent, compute_jacobian_logits,
                              post_trans, post_trans_diff, config['device'],jacobians=False,
...
logits_data_dir = os.path.join(root_dir, 'data', f'{model_name}_Logits', config['metric'])+os.sep
logits_fnames = get_fnames(logits_data_dir, var_names, split_names)
...
if missing:
    if not config["generate_vars"]:
        print(
            "Cached logits variables not found (or empty). Re-running variable extraction now. "
    ...
    if missing:
        details = [f"logits cache: {data_dir}"] + [f"  - missing {s}/{v}" for s, v in missing]
        raise FileNotFoundError(
            "Cached logits tensors are missing or empty.\n"
            + "\n".join(details)
            + "\n"
...
results_dir = get_results_dir(
    "2D", dataset_name, config["metric"], f"{arch}_results_complete2_{config['alpha']}"
)
...
for i in range(0,100):
    ...
    #     if os.path.exists(f'./results/raw_{config['alpha']}/raw_{config['dataset']}_{method}_{i}.pkl')==False:
    #         print(f'./results/raw_{config['alpha']}/raw_{config['dataset']}_{method}_{i}.pkl')
    ...
    # COMPASSG_configs={'fnames':fnames,'model':model,'channel':0,'forward_latent':forward_latent,
    #                  'post_trans':post_trans,'device':config['device'],'beta_range':1e15,
    #                   'alpha':0.1,'sigma':1.0,'kernel_size':1,
    #                   'search_threshold':1e-5,'max_search_steps':1000,'return_scores':False}
    # compassg_len,compassg_cov=COMPASS_Gaussian(sym=True, weights=None, **COMPASSG_configs)
    # acompassg_len,acompassg_cov=COMPASS_Gaussian(sym=False, weights=None,**COMPASSG_configs)
    # print(f'COMPASS-G Len: {compassg_len} COMPASS-G Cov: {compassg_cov}')
    # print(f'A-COMPASS-G Len: {acompassg_len} A-COMPASS-G Cov: {acompassg_cov}')
    # COMPASSF_configs={'fnames':fnames,'model':model,'channel':0,'forward_latent':forward_latent,
    #                  'post_trans':post_trans,'beta_range':1e15,'alpha':0.1,'bandwidth':3,
    #                   'search_threshold':1e-5,'max_search_steps':1000,'return_scores':False,
    #                  'device':config['device']}
    # compassf_len,compassf_cov=COMPASS_Fourier(sym=True, weights=None, **COMPASSF_configs)
    # acompassf_len,acompassf_cov=COMPASS_Fourier(sym=False, weights=None, **COMPASSF_configs)
    # print(f'COMPASS-F Len: {compassg_len} COMPASS-F Cov: {compassg_cov}')
    # print(f'A-COMPASS-F Len: {acompassg_len} A-COMPASS-F Cov: {acompassg_cov}')
    
    # run_data.extend([['COMPASS-G',compassg_len,compassg_cov],
    #                  ['A-COMPASS-G',acompassg_len,acompassg_cov],
    #                  ['COMPASS-F',compassf_len,compassf_cov],
    #                  ['A-COMPASS-F',acompassf_len,acompassf_cov],
    #                 ])
