...
root_dir='/scratch/yc130/ham10k/'
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
model_dir=f'{root_dir}models/{arch}_{str(timestamp)}/'
if os.path.exists(model_dir)==False:
    os.mkdir(model_dir)
img_dir=f'{root_dir}HAM10000_imgs/'
seg_dir=f'{root_dir}HAM10000_segs/'
csv_file=f'{root_dir}HAM10000_metadata.csv'
size=128
batch_size=32
shuffle=True
num_workers=16
...
if shift==None:
    train_set=ebhi(img_dir=f'{root_dir}imagesTr/',
                   seg_dir=f'{root_dir}labelsTr/',size=128,transform=True)
    val_set=ebhi(img_dir=f'{root_dir}imagesVa/',
                 seg_dir=f'{root_dir}labelsVa/',size=128,transform=False)
    test_set=ebhi(img_dir=f'{root_dir}imagesTs/',
                  seg_dir=f'{root_dir}labelsTs/',size=128,transform=False)
else:
