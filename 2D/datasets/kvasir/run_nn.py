...
data_dir='/scratch/yc130/Kvasir-SEG/'
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
root_dir=f'{data_dir}models/{arch}_{str(timestamp)}/'
...
train_set=kvasir(img_dir=f'{data_dir}imagesTr/',seg_dir=f'{data_dir}labelsTr/',transform=True,size=128)
val_set=kvasir(img_dir=f'{data_dir}imagesVa/',seg_dir=f'{data_dir}labelsVa/',transform=False,size=128)
test_set=kvasir(img_dir=f'{data_dir}imagesTs/',seg_dir=f'{data_dir}labelsTs/',transform=False,size=128)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        dice_metric.reset()
        model_path=f'{root_dir}model_{str(epoch)}.pth'
        torch.save(model.state_dict(),model_path)
        if avg_vloss<best_vloss:
             best_vloss=avg_vloss
             model_path=f'{root_dir}best_model.pth'
             torch.save(model.state_dict(),model_path)
